/*
 * levvy: a levenshtein-style fuzzy match distance with a streak bias,
 * i.e. consecutive matched characters are rewarded. Lower is better.
 *
 * Operations and their costs (all parameters are non-negative integers):
 *   - del: delete a character from the query
 *   - skip: skip a character of the haystack line
 *   - sub: substitute a query character for a haystack character
 *   - match: free; each *consecutive* match after the first subtracts streak_bias
 *
 * Lines of a file are padded to the longest line; padding costs
 * padding * length_bias. length_bias defaults to skip_cost, which is the
 * historical behavior (padding was literally skipped characters). Passing
 * negative padding (e.g. -h.length with length_bias == skip_cost) normalizes
 * scores so a perfect substring match ranks equal to an exact match,
 * independent of line length -- see notes.norg and property.test.ts.
 *
 * To keep the numbers non-negative (needed for the u16 zig port), a bias of
 * min(q.length, h.length) * streak_bias is folded into the base cases and
 * compensated at the end; a perfect match scores exactly 0.
 */

export type LevvyParams = {
  del_cost: number;
  skip_cost: number;
  sub_cost: number;
  streak_bias: number;
  length_bias: number; // cost per unit of padding
};

export const default_params: LevvyParams = {
  del_cost: 2,
  skip_cost: 2,
  sub_cost: 3,
  streak_bias: 3,
  length_bias: 2, // == skip_cost: the historical padding behavior
};

export const case_setting: 0 | 1 | 2 = 1 as any; // 0 means exact match, 1 means smart and 2 insensitive

const adjustedChars = (q: string, q_i: number, h: string, h_i: number): [number, number] => {
...
export const referenceLevvy = (
  ...
  params: LevvyParams = default_params,
): number => {
  const { del_cost, skip_cost, sub_cost, streak_bias, length_bias } = params;
  ...
  if (h_len === 0) {
    const dist = q_len * del_cost + padding * length_bias + bias;
    c.set(hash, dist);
    return dist;
  }

  if (q_len === 0) {
    const dist = h_len * skip_cost + padding * length_bias + bias;
    c.set(hash, dist);
    return dist;
  }

  const [a, b] = adjustedChars(q, q_i, h, h_i);

  const is_match = a === b;
  if (is_match) {
    let skip = referenceLevvy(c, q, q_i, h, h_i + 1, padding, false, params) + skip_cost;
    let del = referenceLevvy(c, q, q_i + 1, h, h_i, padding, consecutive_match, params) + del_cost;
    let match = referenceLevvy(c, q, q_i + 1, h, h_i + 1, padding, true, params) - (consecutive_match ? streak_bias : 0);
  ...
  let skip = referenceLevvy(c, q, q_i, h, h_i + 1, padding, false, params) + skip_cost;
  let del = referenceLevvy(c, q, q_i + 1, h, h_i, padding, consecutive_match, params) + del_cost;
  let sub = referenceLevvy(c, q, q_i + 1, h, h_i + 1, padding, false, params) + sub_cost;
...
export const iterativeLevvy = (
  ...
  for (let q_i = 0; q_i <= q_len; q_i++) {
    const dist = (q_len - q_i) * del_cost + padding * length_bias + bias;
    dp[q_i * BH + h_len * B + 0] = dist;
    dp[q_i * BH + h_len * B + 1] = dist;
  }
  for (let h_i = 0; h_i <= h_len; h_i++) {
    const dist = (h_len - h_i) * skip_cost + padding * length_bias + bias;
...
export const iterativeLevvy_fast = (
  q: string, h: string,
  padding: number,
  dp_current: number[], dp_previous: number[],
  params: LevvyParams = default_params,
): number => {
  const { del_cost, skip_cost, sub_cost, streak_bias, length_bias } = params;
  ...
  const padding_cost = padding * length_bias;
  ...
  return dp_previous[0] - (bias > 0 ? params.streak_bias : 0);
};
...
export function path(
  ...
  const { del_cost, skip_cost, sub_cost, streak_bias } = params;
